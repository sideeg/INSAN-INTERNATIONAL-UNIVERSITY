...
<div class="bg-navy-900 text-white py-2 text-sm border-b border-navy-700">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        <div class="flex items-center gap-6">
            ...
            <a href="tel:{{ config('university.phone', '+249123456789') }}" class="flex items-center gap-2 hover:text-gold-500 transition-colors">
                <i class="fas fa-phone text-gold-500 text-xs"></i>
                <span>{{ config('university.phone', '+249 123 456 789') }}</span>
            </a>
        </div>
        <div class="flex items-center gap-4">
            <a href="{{ route('student.portal') }}" class="flex items-center gap-2 hover:text-gold-500 transition-colors">
